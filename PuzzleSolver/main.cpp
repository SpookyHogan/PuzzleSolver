...
int main()
{
    ...
    TwoByTwoIndex indecies[2000];

    ThreeByThreeIndex threes[10500];
...

    {
        ...
        for (int i = 0; i < setIndex; i++)
        {
            for (int TRI = 0; TRI < 16; TRI++)
            {
                if (TRI != indecies[i].TLIndex && TRI != indecies[i].TRIndex && TRI != indecies[i].BLIndex && TRI != indecies[i].BRIndex)// if it is a new piece
                {
                    for (int r0 = 0; r0 < 4; r0++)
                    {
                        if (checkFit(pieces[indecies[i].TRIndex].getRight(indecies[i].TRRot), pieces[TRI].getLeft(r0)))// top right fits
                        {
                            for (int MRI = 0; MRI < 16; MRI++)
                            {
                                if (MRI != indecies[i].TLIndex && MRI != indecies[i].TRIndex && MRI != indecies[i].BLIndex && MRI != indecies[i].BRIndex
                                    && MRI != TRI)// if it is a new piece
                                {
                                    for (int r1 = 0; r1 < 4; r1++)
                                    {
                                        if (checkFit(pieces[indecies[i].BRIndex].getRight(indecies[i].BRRot), pieces[MRI].getLeft(r1))
                                            && checkFit(pieces[TRI].getDown(r0), pieces[MRI].getUp(r1))) // mid right fits
                                        {
                                            for (int BRI = 0; BRI < 16; BRI++)
                                            {
                                                if (BRI != indecies[i].TLIndex && BRI != indecies[i].TRIndex && BRI != indecies[i].BLIndex && BRI != indecies[i].BRIndex
                                                    && BRI != MRI && BRI != TRI)// if is a new piece
                                                {
                                                    for (int r2 = 0; r2 < 4; r2++)
                                                    {
                                                        if (checkFit(pieces[MRI].getDown(r1), pieces[BRI].getUp(r2)))// bottom right fits
                                                        {
                                                            ...
                                                            {
                                                                if (BMI != indecies[i].TLIndex && BMI != indecies[i].TRIndex && BMI != indecies[i].BLIndex && BMI != indecies[i].BRIndex
                                                                    && BMI != BRI && BMI != MRI && BMI != TRI) // piece is new
                                                                {
                                                                    for (int r3 = 0; r3 < 4; r3++)
                                                                    {
                                                                        if (checkFit(pieces[indecies[i].BRIndex].getDown(indecies[i].BRRot), pieces[BMI].getUp(r3))
                                                                            && checkFit(pieces[BRI].getLeft(r2), pieces[BMI].getRight(r3)))// bottom middle fits
                                                                        {
                                                                            for (int BLI = 0; BLI < 16; BLI++)
                                                                            {
                                                                                if (BLI != indecies[i].TLIndex && BLI != indecies[i].TRIndex && BLI != indecies[i].BLIndex && BLI != indecies[i].BRIndex
                                                                                    && BLI != BMI && BLI != BRI && BLI != MRI && BLI != TRI)// the piece is new
                                                                                {
                                                                                    for (int r4 = 0; r4 < 4; r4++)
                                                                                    {
                                                                                        if (checkFit(pieces[indecies[i].BLIndex].getDown(indecies[i].BLRot), pieces[BLI].getUp(r4))
                                                                                            && checkFit(pieces[BMI].getLeft(r3), pieces[BLI].getRight(r4)))// the piece fits, so does the whole
                                                                                        {
                                                                                            ...
                                                                                            //else
                                                                                            {
                                                                                                threes[threesIndex] = { indecies[i], TRI, r0, MRI, r1, BRI, r2, BMI, r3, BLI, r4 };
                                                                                                threesIndex++;
                                                                                            }
                                                                                        }
                                                                                    }
...
bool checkTwoByTwo(Piece tl, Piece tr, Piece bl, Piece br)
{
    ...
    if (abs(int(tl.getRight()) - int(tr.getLeft()) != 1))
    {
        return false;
    }

    if (abs(int(tr.getDown()) - int(br.getUp())) != 1)
    {
        return false;
    }

    if (abs(int(br.getLeft()) - int(bl.getRight())) != 1)
    {
        return false;
    }

    if (abs(int(bl.getUp()) - int(tl.getDown())) != 1)
    {
        return false;
    }
